"""Tests for arXiv-first behavior with INSPIRE-HEP fallback."""
...
class InspireFallbackTests(unittest.TestCase):
    def test_fetch_today_uses_arxiv_when_available(self) -> None:
        fake = [{"id": "1234.5678", "title": "T", "abstract": "", "url": "u", "authors": []}]
        with mock.patch("cuhkvoting.cli._arxiv_query", return_value=fake) as arxiv:
            with mock.patch("cuhkvoting.cli._inspire_query_retry") as inspire:
                out = cli._fetch_today_entries(["gr-qc"])
        self.assertEqual(out, fake)
        self.assertEqual(arxiv.call_count, 1)
        self.assertEqual(inspire.call_count, 0)

    def test_fetch_today_falls_back_to_inspire_when_arxiv_down(self) -> None:
        fake_inspire = [{"id": "9999.0001", "title": "I", "abstract": "x", "url": "u", "authors": []}]
        with mock.patch("cuhkvoting.cli._arxiv_query", side_effect=TimeoutError("arXiv down")):
            with mock.patch("cuhkvoting.cli._inspire_query_retry", return_value=fake_inspire) as inspire:
                out = cli._fetch_today_entries(["gr-qc"])
        self.assertEqual(out, fake_inspire)
        self.assertEqual(inspire.call_count, 1)

    def test_fetch_entries_builds_arxiv_submittedDate_query(self) -> None:
    ...
    def test_fetch_entries_fallback_builds_inspire_query(self) -> None:
        start = dt.date(2026, 5, 1)
        end = dt.date(2026, 5, 2)
        with mock.patch("cuhkvoting.cli._arxiv_query", side_effect=TimeoutError("arXiv down")):
    ...
    def test_validate_prefers_arxiv_record(self) -> None:
        ...
        with mock.patch("cuhkvoting.cli._arxiv_query", return_value=[arxiv_entry]) as arxiv:
            with mock.patch("cuhkvoting.cli._inspire_get_by_arxiv_id") as get:
                out = cli._validate_arxiv_entry("2601.09678v2")
        self.assertEqual(out, arxiv_entry)
        self.assertEqual(arxiv.call_count, 1)
        self.assertEqual(get.call_count, 0)

    def test_validate_falls_back_to_inspire_when_arxiv_down(self) -> None:
    ...
    def test_validate_arxiv_empty_is_typo_guard(self) -> None:
        # arXiv reachable but no hit → hard "not found" (typo guard), INSPIRE not consulted.
        with mock.patch("cuhkvoting.cli._arxiv_query", return_value=[]):
            with mock.patch("cuhkvoting.cli._inspire_get_by_arxiv_id") as get:
                with self.assertRaises(SystemExit):
                    cli._validate_arxiv_entry("2601.09678")
        self.assertEqual(get.call_count, 0)

    def test_validate_reraises_when_arxiv_down_and_inspire_missing(self) -> None:
        # arXiv unreachable AND INSPIRE has no record → re-raise the original network error.
        with mock.patch("cuhkvoting.cli._arxiv_query", side_effect=TimeoutError("arXiv down")):
