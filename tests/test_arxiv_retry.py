    ...
    def test_stops_after_max_retries(self) -> None:
        ...
        with mock.patch("cuhkvoting.cli._http_text", side_effect=fail_http_text):
            with mock.patch("cuhkvoting.cli.time.sleep", return_value=None):
                # On exhaustion _arxiv_query re-raises the original network error (not SystemExit),
                # so callers' INSPIRE / stale-cache fallbacks (which catch the URLError family) fire.
